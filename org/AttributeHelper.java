...
public class AttributeHelper {
	...
	public static void showInBrowser(String url) {
		...
		// this is not save, in case "&amp;" was included before & was exchanged with &amp; by this application
		// the replacement is needed because of the GML file loader, which can not load a file which contains a &, but
					...
					try {
						...
						JOptionPane.showMessageDialog(null,
								"<html>Error executing command. Error Messages:<p>"
										+ e.getLocalizedMessage() + " (Windows File Open)<p>"
										+ e2.getLocalizedMessage() + " (gnome File Open)<p>"
...
//	    System.setProperty("com.apple.mrj.application.apple.menu.about.name", applicationName);
	...
	public static String getDefaultAttributeDescriptionFor(String id, String tabName, Attribute a) {
		...
		if (!idInit) initNiceIds();
		// look for an attribute name like test1, test2, hallo1, ...
		// the name ends with a number...
		// in this case the number is removed, the lookup for the known "friendly names"
...
//			}
		...
		if (true) {
			if (endNumber!=null && endNumber.length()>0 && result.contains(":")) {
				...
				if (ReleaseInfo.getRunningReleaseStatus()==Release.KGML_EDITOR)
					result = result.substring(0, idx)+ " (list):" + "<html><!-- "+endNumber+" -->"+ErrorMsg.stringReplace(endS, "<html>", "");
				else {
					if (result.substring(0, idx).length()>0)
						result = result.substring(0, idx)+ ":" + "<html><!-- "+endNumber+" -->"+ErrorMsg.stringReplace(endS, "<html>", "");
					else
						result = "<html><!-- "+endNumber+" -->"+ErrorMsg.stringReplace(ErrorMsg.stringReplace(endS, "&nbsp;", " "), "<html>", "").trim();
				}
			}
		}
		return result;	
	...
	private static void initNiceIds() {
		...
		idToNiceId.put("empty_border_width", chartSelN+": Spacing around Chart");
		...
		idToNiceId.put("background_color", chartSelN+":<html>&nbsp;Background-Color<br>&nbsp;<small><font color=\"gray\">(black=translucent)");
		...
		idToNiceId.put("node_categoryBackgroundColorB", chartDiagram+":<html>&nbsp;Background Color<br>&nbsp;<small><font color=\"gray\">(black=translucent)");
	...
	public static void setLabel(Edge edge, String label, String fontName) {
		...
		try {
			...
			if (hasAttribute(edge, GraphicAttributeConstants.LABELGRAPHICS)) {
				...
				labelAttr = new EdgeLabelAttribute(GraphicAttributeConstants.LABELGRAPHICS,
							label);
				edge.addAttribute(labelAttr,
							GraphicAttributeConstants.LABEL_ATTRIBUTE_PATH);
			}

			if (fontName != null) setFont(labelAttr, fontName);
	...
	public static void setLabel(Node node, String label, String fontName, String alignment) {
		...
		try {
			...
			labelAttr.setLabel(label);
			if (alignment!=null) labelAttr.setAlignment(alignment);
		} catch (Exception ex) {
	...
	public static void setFont(LabelAttribute label, String fontName) {
		if (label != null && fontName != null) label.setFontName(fontName);
	}
...
/*
	 ...
	 * String) the attribute values are set with "standard attributes". If it has
	 * a unknown type a <code>ObjectAttribute</code> is added or set. These
	...
	public static void setAttribute(Attributable attributable, String path,
		...
		if (attributeValue instanceof Double) {
			try {
				if (((Double) attributeValue).doubleValue() == Double.NaN) return;
		...
		if (attributeValue instanceof Float) {
			if (((Float) attributeValue).floatValue() == Float.NaN) return;
		...
		if (attributeValue instanceof Long) {
			if (((Long) attributeValue).intValue() == Long.MAX_VALUE) return;
		...
		if (attributeValue instanceof Short) {
			if (((Short) attributeValue).shortValue() == Short.MAX_VALUE) return;
...
//		String oldSubstanceName = (String) getAttributeValue(node, "data", "substancename",
//					null, new String(""));
...
//						+ e.getLocalizedMessage());
	...
	public static void setBorderWidth(Node node, double frameThickness) {
      NodeGraphicAttribute na =
         (NodeGraphicAttribute) node.getAttribute(GraphicAttributeConstants.GRAPHICS);
	...
	/**
	 * @param attr
	 * @param hexStr A color defined like #FFFFFF (must be 6 characters (hex) for the color components)
	 */
...
//		sbmlEncodedLabel = ErrorMsg.stringReplace(sbmlEncodedLabel, "_alpha_", "&alpha;");
//		sbmlEncodedLabel = ErrorMsg.stringReplace(sbmlEncodedLabel, "_beta_", "&beta;");
...
//	};
	 ...
	 private static final String[] knownColorNames = {
		    "alice blue",            // F0F8FF
	...
	public static String getHelpTopicFor(String tabName, String groupName) {
		if (!ReleaseInfo.getIsAllowedFeature(FeatureSet.GravistoJavaHelp))
			return null;
		if (helpTopics==null) initHelpTopics();
	...
	public static void copyReplaceStringAttribute(Attributable a, String path, String name, String copyTo, String searchString, String replaceString) {
		if (hasAttribute(a, path, name)) {
			...
			if (attVal.length()>0) {
				attVal = ErrorMsg.stringReplace(attVal, searchString, replaceString);
	...
	public static void setLabel(GraphElement ge, String label) {
		if (ge instanceof Node)
			setLabel((Node)ge, label); else
		if (ge instanceof Edge)
			setLabel((Edge)ge, label); else
	 ...
	 * In case the name ends with "*", all attributes with a name, which starts with the same
	 * string, will be removed. Otherwise a single attribute (if available) with the exact name
	 * as given, is removed.
	 * @param attributable
	 * @param path
	 * @param name A name of a single attribute "test" or a group of attributes "test*" (test1, test2, testA, testABC, ...).
	 * @return True, if the removal was successful. Fals, if a error has occured, which is added to the system log. 
	 */
	...
	public static void setLabelColor(Node n, Color color) {
		try {
			HashMapAttribute l = (HashMapAttribute) n.getAttribute(NodeLabelAttribute.LABEL_ATTRIBUTE_PATH+"."+NodeLabelAttribute.LABELGRAPHICS);
	...
	public static void addEdgeBends(Edge edge, Collection<Vector2d> points) {
		try {
			...
			for (Vector2d point : points) {
				...
				ega.getBends().add(	new CoordinateAttribute("bend"+cnt, point.x, point.y), false);
			}
		} catch(Exception e) {
	...
	public static String getNiceEdgeOrNodeLabel(GraphElement ge, String nodeLabelIfMissing) {
		if (ge instanceof Edge) {
			...
			return AttributeHelper.getLabel(e, 
					AttributeHelper.getLabel(e.getSource(), nodeLabelIfMissing)
					+(e.isDirected() ? "->" : "--")+
	...
	public static double getPositionZ(Node a, boolean setDefault) {
		double z = (Double)getAttributeValue(a, "graphics", "z_", new Double(0), new Double(0), setDefault);
		return z;
	}
